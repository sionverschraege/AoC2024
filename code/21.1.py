...
for a in dirpad.keys():
    for b in dirpad.keys():
        costs[0][a + b] = fastest(None, a+b, dirpad)

for i in range(1):
    costs.append({})
    for a in dirpad.keys():
        for b in dirpad.keys():
            costs[-1][a + b] = fastest(costs[-2], a+b, dirpad)

total = 0
for line in lines:
    cost = fastest(costs[-1],'A' + line[0],numpad) + sum([fastest(costs[-1],line[i:i+2],numpad) for i in range(len(line) - 1)])
